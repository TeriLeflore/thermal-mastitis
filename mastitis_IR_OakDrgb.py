...
# 3/26 Simplify code by deleting one pipeline|8 bit image isnt working consistently Error: [ WARN:0@29.240] global cap_v4l.cpp:1134 tryIoctl VIDEOIO(V4L2:/dev/v4l/by-id/usb-FLIR_Boson_196764-video-index0): select() timeout.
...
device2 = dai.Device(pipeline, usb2Mode=True) #communicate vid USB2
...
q2 = device2.getOutputQueue(name="disparity", maxSize= 8, blocking=False)
qRGB2 = device2.getOutputQueue(name="rgb", maxSize= 8, blocking=False)
...
async def run():
	...
	# Loop through the IR cameras collecting 1 unscaled 16-bit and scaled 8-bit images
     ...
     # the tryget method return latest results or none if the queue is empty   
        inDisparity = q.get() 
        ...
        cv2.imwrite("_disparity_" + img_name + ".png", frame)
        
        frame2 = cv2.applyColorMap(frame2, cv2.COLORMAP_JET)
        # cv2.imshow("disparity_color", frame2)
        cv2.imwrite("_disparity2_" + img_name + ".png", frame2)
...
#3/26 Simplify code by deleting one of the pipelines
